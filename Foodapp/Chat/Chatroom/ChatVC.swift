...
class ChatVC: UIViewController,UITextFieldDelegate {
    ...
    func fetchMessages() {
                    ...
                    dict4.setObject(dic["fromUserId"] as Any, forKey: "fromUserId" as NSCopying)
                    dict4.setObject(dic["text"] as Any, forKey: "text" as NSCopying)
                    dict4.setObject(dic["timestamp"] as Any, forKey: "timestamp" as NSCopying)
                    dict4.setObject(dic["name"] as Any, forKey: "name" as NSCopying)
                    print(dict4)

                    self.arrMsg.add(dict4)
                    ...
                    //時間戳記排序
                    let dates = NSSortDescriptor(key: "timestamp", ascending: true)
                    let sortedarrMsg = self.arrMsg.sortedArray(using: [dates])
                    ...
                    for i in 0 ..< self.arrMsg.count{
                        var dicdata  = self.arrMsg[i] as! [String : Any]
                        
                        let dateConverted = String(describing: dicdata["timestamp"]!)
                        
                        let date = Date(timeIntervalSince1970: Double(dateConverted)!)
                        
                        dicdata["timestamp"] = date
...
//                dict.setObject(data["fromUserId"] as Any, forKey: "fromUserId" as NSCopying)
//                dict.setObject(data["text"] as Any, forKey: "text" as NSCopying)
//                dict.setObject(data["timestamp"] as Any, forKey: "timestamp" as NSCopying)
//                dict.setObject(data["name"] as Any, forKey: "name" as NSCopying)
                
//                self.arrMsg.add(dict)
